...
function DetalleCompra() {
  ...
  useEffect(() => {
    const fetchFactura = async () => {
      try {
        const res = await compraService.getById(numero);
        setFactura(res.data?.data || res.data || null);
      } catch (err) {
  ...
  return (
    <Container className="mt-4">
      ...
      <Card className="p-3 mb-3 bg-light">
        ...
        <Row className="mb-2">
          <Col md={4}>
            <strong>Base IVA:</strong> {factura.baseIva} €
          </Col>
          <Col md={4}>
            <strong>Total IVA:</strong> {factura.totalIva} €
          </Col>
          <Col md={4}>
        ...
        <Row>
          <Col md={4}>
            ...
            {factura.baseIva
              ? ((factura.totalIva / factura.baseIva) * 100).toFixed(2)
              : "0"}
            %
          </Col>
        </Row>
      ...
      <Card className="p-3 mb-3 bg-light">
        ...
        <Row className="mb-2">
          <Col md={6}>
            <strong>Total Asignado:</strong> {factura.totalAsignado} €
          </Col>
          <Col md={6}>
            <strong>Porcentaje Asignado:</strong> {porcentajeAsignado} %
          </Col>
        </Row>
      </Card>
      ...
      <Card className="p-3 mb-3">
        <Form.Group>
          ...
          <Form.Control
            as="textarea"
            rows={3}
            value={factura.observaciones || "Sin observaciones"}
            readOnly
          />
        </Form.Group>
      </Card>
